...
void Agency::sortByMake()   // alphabetical
{
	...
	Car *tempHome = tempInventory;
	Car *inventoryHome = inventory; 
	int pointerCount, pointerCount2; 

	//Creates a temp array to store sorted items 
	char *tempArray = new char[CAR_COUNT];
	char *tempArrayPtr = tempArray; 
	char *tempArrayHome = tempArrayPtr;
	bool swapped = true; 
	
	tempInventory++; //Increments the tempInventory to second Position 
	

	cout << "First element in inventory: " << (*inventory).getMake() << endl;
	cout << "First element in temp inventory: " << (*tempInventory).getMake() << endl;

	while (swapped != false){

	//This for loop is to fill temp array
	for (pointerCount = 0; pointerCount < CAR_COUNT; pointerCount++){
		//This loop is to file through (*inventory).make & (*inventory).model to alphabetize 
		for (pointerCount2 = 0; pointerCount2 < CAR_COUNT - 1; pointerCount2++){
				
				swapped = false; 

				tempArrayPtr = (*inventory).getMake();
				
				if (tempArrayPtr < (*tempInventory).getMake()){
					tempArrayPtr = (*tempInventory).getMake(); 
					swapped = true; 
				}
				cout << "Element in sorted array: " << tempArrayPtr << endl;
				inventory++;
				tempInventory++; 
				tempArrayPtr++; 
		}
